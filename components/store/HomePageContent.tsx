...
export default function HomePageContent({ banners, featured, newArrivals }: Props) {
  ...
  return (
    <div className="min-h-screen">
      <section className="bg-white border-b border-[#e8e0c8]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-10">
          <motion.div
            initial={{ opacity: 0, y: 12 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.5 }}
            className="flex flex-col md:flex-row items-center justify-center md:justify-between gap-6 md:gap-10"
          >
            <BrandLogo href={undefined} size="lg" />
            <div className={`max-w-xl text-center ${locale === 'ar' ? 'md:text-right' : 'md:text-left'}`}>
              <p className="text-[#9a7b2f] text-xs sm:text-sm font-semibold uppercase tracking-[0.2em] mb-2">
                {t('home.brand.eyebrow')}
              </p>
              <h1 className="text-2xl sm:text-3xl font-extrabold text-[#1a1408] tracking-tight mb-2">
                {t('home.brand.title')}
              </h1>
              <p className="text-[#5c5346] text-sm sm:text-base leading-relaxed mb-5">
                {t('home.brand.sub')}
              </p>
              <Link
                href="/products"
                className="inline-flex items-center gap-2 bg-[#c9a84c] hover:bg-[#b8963f] text-[#1a1408] font-bold px-5 py-2.5 rounded-xl transition-colors"
              >
                {t('home.brand.cta')}
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-12 animate-fade-up">
        <div className="relative overflow-hidden rounded-3xl border border-[var(--border-card)] bg-[#0D1628] p-8 md:p-12">
          ...
          <div className="relative z-10 max-w-lg">
            ...
            <Link
              href="/products"
              className="inline-flex items-center gap-2 bg-[var(--cyan)] hover:bg-[#b8963f] text-[var(--text-on-badge)] font-bold px-6 py-3 rounded-xl transition-all hover:shadow-glow"
            >
              {t('home.promo.cta')}
            </Link>
          </div>
